...
var TransactionBuilder = class {
  ...
  async signAndSend() {
    ...
    const { msgId, blockHash } = await new Promise((resolve, reject) => this._tx.signAndSend(this._account, this._signerOptions, ({ events, status }) => {
      if (status.isInBlock) {
        let msgId2;
        events.forEach(({ event }) => {
          ...
          if (method === "MessageQueued" && section === "gear") {
            ...
            resolve({ msgId: msgId2, blockHash: status.asInBlock.toHex() });
    ...
    return {
      ...
      response: async (rawResult = false) => {
        ...
        return this._registry.createType(`(String, String, ${this._responseType})`, message.payload)[2][getPayloadMethod(this._responseType)]();
      }
    };
  }
};
    ...
    for (const func of service.funcs) {
      ...
      if (func.isQuery) {
        queries[func.name] = async (origin, value = 0n, atBlock, ...args) => {
          ...
          const payload = this.registry.createType(`(String, String, ${params.map((p) => p.type).join(", ")})`, [service.name, func.name, ...args]).toHex();
...
//# sourceMappingURL=chunk-BIIQFJY5.js.map
